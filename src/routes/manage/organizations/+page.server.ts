...
import type { ApiResponse, IUnit, OrganizationDetails } from "$lib/types/types";
...
export const load: PageServerLoad = async ({ url, locals }) => {
    ...
    const organizationDetailsRes = await fetch(`${SERVER_URL}/Users/GetOrganizationDetails`, {
        headers: {
    ...
    if (!organizationDetailsRes.ok && groupDetailsRes.ok) {
        throw new Error('Unable to fetch organization records');
    }
    const organizationDetails: ApiResponse<OrganizationDetails> = await organizationDetailsRes.json();
    ...
    return {
        ...
        organizationStructure: JSON.parse(organizationDetails?.data?.orgStructure || "{}"),
    };
};
